/// @description text_draw_text(textbox)
/// @param textbox
...
for(k=0; k<ds_list_size(lines); k+=1) {
    ...
    if (sumchars < textbox[? "__animatedchars"] and line[? "text"] != "{NL}") {
        ...
        if (sumchars+string_length(line[? "text"]) < textbox[? "__animatedchars"]) {
            draw_text_transformed(Tx+Tspace_left+buffer,Ty+Tspace_top+(Tspace_lineheight*line_count),string_hash_to_newline(line[? "text"]),1,line[? "__height"],0);
            var tw = string_width(string_hash_to_newline(line[? "text"]));
            buffer+=tw;
        } else {
            //or maybe not?
            var remaining = real(textbox[? "__animatedchars"])-sumchars;
            draw_text_transformed(Tx+Tspace_left+buffer,Ty+Tspace_top+(Tspace_lineheight*line_count),string_hash_to_newline(string_copy(line[? "text"], 1, remaining)),1,line[? "__height"],0);
            var tw = string_width(string_hash_to_newline(string_copy(line[? "text"], 1, remaining)));
            buffer+=tw;
        }
